...
def eer_from_scores(scores):
    ...
    # for i, rate in enumerate(far):
        # print('i = {}, far: {}, frr: {}, threshold = {}'.format(i, rate,
            # frr[i], thresholds[i]))
...
class ClassifierFixedText(object):
    ...
    def fit(self, Xi, labels=None):
        ...
        if file_name is not None and os.path.isfile(file_name):
            ...
            with open(file_name, 'rb') as handle:
                ...
                # print_verbose('loaded pickle! ' + file_name, self.params.verbose)
    ...
    def score(self, X, save_stats=False):
        ...
        for i, Xi in enumerate(X):
            ...
            # add this to stats
            # row = []
            # row.append(np.copy(Xi)) # Xi will be changed later
    ...
    def _normalize_features(self, X):
        ...
        FIXME: Replace this by my more understandable loops from dbn?
        """
...
def run_classifier_kfold(cl, X, Y, score_norm):
    '''
    Swipes guys use this method. It might also be more suitable for the
    keystrokes data?

    Uses sklearn's cross_validate method.
    
    90% - 10% train - test combination for android swipes.

    TODO: Deal with the case of one class guys - can this general method be
    used for the stuff done in validation as well?

    Just implemented this in run_classifier itself.
    '''
    pass
...
def run_classifier(cl, X, impostor_samples, seed, score_normalization, 
    ...
    if cl.params.android: 
        ...
        # train_samples, test_samples = train_test_split(X, test_size =
                # android_test_size, random_state=1234)
        train_samples, test_samples = train_test_split(X, test_size =
        ...
        # impostor_train, impostor_samples = train_test_split(impostor_samples,
                # test_size = android_test_size, random_state=1234)
        impostor_train, impostor_samples = train_test_split(impostor_samples,
    ...
    # Splitting it into two equal halves - as done in the CMU paper etc.    
        ...
        # print(impostor_samples[0])
        # print(test_samples[0])
    else:
        assert False, 'has to be keystrokes or android'
    ...
    # FIXME: Make this simpler. Check vivek's implementation - as long as it
...
def _normalize_scores(scores, score_normalization, cl):
    ...
    if score_normalization is not None:
        ...
        # print('max score: ', cl.max_score)
        # print('min score: ', cl.min_score)
